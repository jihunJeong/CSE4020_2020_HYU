...
def exp(rv):
    ...
    resultM = np.identity(3)
    resultM[0][0] = np.cos(th) + (ux**2)*(1-np.cos(th))
    resultM[0][1] = ux*uy*(1-np.cos(th))-uz*(np.sin(th))
    resultM[0][2] = ux*uz*(1-np.cos(th))+uy*(np.sin(th))
    resultM[1][0] = uy*ux*(1-np.cos(th))+uz*(np.sin(th))
    resultM[1][1] = np.cos(th) + (uy**2)*(1-np.cos(th))
    resultM[1][2] = uy*uz*(1-np.cos(th))-ux*(np.sin(th))
    resultM[2][0] = uz*ux*(1-np.cos(th))-uy*(np.sin(th))
    resultM[2][1] = uz*uy*(1-np.cos(th))+ux*(np.sin(th))    
    resultM[2][2] = np.cos(th) + (uz**2)*(1-np.cos(th))
...
#Implement interpolateRotVec
def interpolateRotVec(rv1, rv2, t):
    resultM = exp(lerp(rv1, rv2, t)) 
    return resultM

#Implement interpolateZYXEulerAngles
def interpolateZYXEuler(euler1, euler2, t):
    return ZYXEulerToRotMat(lerp(euler1, euler2, t))   

#Implement interpolateRotMat
